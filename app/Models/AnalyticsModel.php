...
class AnalyticsModel extends Model
{
     ...
     */
    public function getAnalyticsStats($clientIds)
    {
        $today = date('Y-m-d');
        $sevenDaysAgo = date('Y-m-d', strtotime('-7 days'));
        
        $clientIdsArray = (array) $clientIds;

        // 1. Visitors Today (Unique Session IDs)
        $visitorsToday = $this->whereIn('client_id', $clientIdsArray)
                             ->where('type', 'visitor')
                             ->where('created_at >=', $today . ' 00:00:00')
                             ->countAllResults();

        // 2. Page Views Today
        $viewsToday = $this->whereIn('client_id', $clientIdsArray)
        ...
        $visitorsLast7 = $this->whereIn('client_id', $clientIdsArray)
                              ->where('type', 'visitor')
                              ->where('created_at >=', $last7DaysStart . ' 00:00:00')
                              ->where('created_at <=', $last7DaysEnd . ' 23:59:59')
                              ->countAllResults() / 7;

        $viewsLast7 = $this->whereIn('client_id', $clientIdsArray)
                            ->where('type', 'page_view')
                            ->where('created_at >=', $last7DaysStart . ' 00:00:00')
                            ->where('created_at <=', $last7DaysEnd . ' 23:59:59')
                            ->countAllResults() / 7;
